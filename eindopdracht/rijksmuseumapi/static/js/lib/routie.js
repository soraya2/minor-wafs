...
var Routie = function (a, b) {
  var c = [],
    ...
    g = function (a, b) {
      ...
      var c = this.regex.exec(a); if (!c) {
        return !1;
      } for (var d = 1, e = c.length; e > d; ++d) {
        var f = this.keys[d - 1],
          g = typeof c[d] === 'string' ? decodeURIComponent(c[d]) : c[d]; f && (this.params[f.name] = g), b.push(g);
      } return !0;
    }, g.prototype.toURL = function (a) {
      var b = this.path; for (var c in a) {
        b = b.replace('/:' + c, '/' + a[c]);
      } if (b = b.replace(/\/:.*\?/g, '/').replace(/\?/g, ''), b.indexOf(':') != -1) {
        throw new Error('missing parameters for url: ' + b);
      } return b;
    }; var h = function (a, b, c, d) {
        return a instanceof RegExp ? a : (Array.isArray(a) && (a = '(' + a.join('|') + ')'), a = a.concat(d ? '' : '/?').replace(/\/\(/g, '(?:/').replace(/\+/g, '__plus__').replace(/(\/)?(\.)?:(\w+)(?:(\(.*?\)))?(\?)?/g, function (a, c, d, e, f, g) {
          return b.push({name: e, optional: Boolean(g)}), c = c || '', String(g ? '' : c) + '(?:' + (g ? c : '') + (d || '') + (f || d && '([^/.]+?)' || '([^/]+?)') + ')' + (g || '');
        }).replace(/([\/.])/g, '\\$1').replace(/__plus__/g, '(.+)').replace(/\*/g, '(.*)'), new RegExp('^' + a + '$', c ? '' : 'i'));
      },
      i = function (a, b) {
        var e = a.split(' '),
          f = e.length == 2 ? e[0] : null; a = e.length == 2 ? e[1] : e[0], d[a] || (d[a] = new g(a, f), c.push(d[a])), d[a].addHandler(b);
      },
      j = function (a, b) {
        if (typeof b === 'function') {
          ...
          typeof b === 'undefined' && j.navigate(a);
        }
      }; j.lookup = function (a, b) {
        for (var d = 0, e = c.length; e > d; d++) {
          var f = c[d]; if (f.name == a) {
            return f.toURL(b);
          }
        }
...
}; typeof module === 'undefined' ? Routie(window) : module.exports = Routie(window, !0);
